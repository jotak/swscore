...
func TraceDetails(w http.ResponseWriter, r *http.Request) {
	...
	traces, code, err := business.Jaeger.GetJaegerTraceDetail(r)
	if err != nil {
		log.Error(err)
		RespondWithError(w, code, err.Error())
		return
...
func getJaegerInfo(requestToken string) (*models.JaegerInfo, int, error) {
	...
	if ha, _ := canAccessURL(jaegerConfig.InClusterURL, &auth); !ha {
		return nil, http.StatusServiceUnavailable, errors.New("Kiali can't access to Jaeger ")
	}
